...
class GoogleNet:
    ...
    @staticmethod
    def build_model(input_shape, NB_CLASS, weights_path=None):
        ...
        model = Model(input=img_input, output=[x], name= MODEL_NAME)

        if weights_path:
            model.load_weights(weights_path)

        # compile model
        opt = SGD(lr= LEARNING_RATE, momentum= MOMENTUM, decay=1e-6, nesterov=True)
